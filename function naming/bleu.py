...
def eval_accuracies(hypotheses, references, sources=None,
    ...
    for row in hypotheses:
        ...
        if len(cols) == 1:
            ...
            (rid, pred) = (cols[0], cols[1]) 
        predictionMap[count] = [splitPuncts(pred.strip().lower())]
        count+=1

    count= 0
    for row in gf:
        (rid, pred) = row.split('\t') 
        if count in predictionMap: # Only insert if the id exists for the method
            if count not in goldMap:
                goldMap[count] = []
            goldMap[count].append(splitPuncts(pred.strip().lower()))
            count+=1
            
    hypotheses = predictionMap
    references = goldMap
    ...
    return precision.avg * 100, recall.avg * 100, f1.avg * 100
